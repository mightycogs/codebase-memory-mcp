...
static const char* python_keywords[] = {
    ...
    "self","cls","__init__","__name__","__main__","super",
...
static const char* js_keywords[] = {
    ...
    "of","static","get","set","from","as","constructor","prototype",
    "console","window","document","process","module","exports","require",
    "Array","Object","String","Number","Boolean","Symbol","Map","Set",
    "Promise","Error","RegExp","Date","Math","JSON","parseInt","parseFloat",
...
static const char* rust_keywords[] = {
    ...
    "Some","None","Ok","Err","Vec","String","Box","Rc","Arc","Option","Result",
    "println","eprintln","format","write","writeln","print","eprint","panic",
    "assert","assert_eq","assert_ne","debug_assert","todo","unimplemented",
    "cfg","derive","test","allow","deny","warn","forbid","deprecated",
    NULL
};
...
static const char* java_keywords[] = {
    ...
    "System","String","Integer","Long","Double","Float","Boolean","Object",
    "List","Map","Set","Optional","Stream","Arrays","Collections",
    NULL
};
...
static const char* generic_keywords[] = {
    ...
    "function","def","fn","func","fun","proc","sub","method",
    "async","await","yield",
    NULL
};
...
bool cbm_is_keyword(const char* name, CBMLanguage lang) {
    ...
    const char** keywords;
    switch (lang) {
        case CBM_LANG_GO:         keywords = go_keywords; break;
        case CBM_LANG_PYTHON:     keywords = python_keywords; break;
...
bool cbm_kind_in_set(TSNode node, const char** types) {
    ...
    for (const char** t = types; *t; t++) {
        if (strcmp(kind, *t) == 0) return true;
    }
    return false;
}
...
bool cbm_has_ancestor_kind(TSNode node, const char* kind, int max_depth) {
    ...
    for (int i = 0; i < max_depth; i++) {
        ...
        if (strcmp(ts_node_type(parent), kind) == 0) return true;
        cur = parent;
    }
    return false;
}
...
static const char* func_kinds_js[] = {"function_declaration","method_definition","arrow_function","function_expression",NULL};
...
static const char** func_kinds_for_lang(CBMLanguage lang) {
    switch (lang) {
        case CBM_LANG_GO:         return func_kinds_go;
        case CBM_LANG_PYTHON:     return func_kinds_python;
        case CBM_LANG_JAVASCRIPT:
        case CBM_LANG_TYPESCRIPT:
        case CBM_LANG_TSX:        return func_kinds_js;
        case CBM_LANG_RUST:       return func_kinds_rust;
...
TSNode cbm_find_enclosing_func(TSNode node, CBMLanguage lang) {
    ...
    for (;;) {
        ...
        for (const char** k = kinds; *k; k++) {
            if (strcmp(pk, *k) == 0) return parent;
        }
        cur = parent;
    }
    TSNode null_node = {0};
...
const char* cbm_enclosing_func_qn(CBMArena* a, TSNode node, CBMLanguage lang,
                                    const char* source, const char* project,
...
const char* cbm_enclosing_func_qn_cached(CBMExtractCtx* ctx, TSNode node) {
    ...
    const char* qn = cbm_enclosing_func_qn(
        ctx->arena, node, ctx->language, ctx->source,
...
bool cbm_is_module_level(TSNode node, CBMLanguage lang) {
    ...
    if (lang == CBM_LANG_PYTHON) {
        if (strcmp(pk, "module") == 0) return true;
    ...
    if (lang == CBM_LANG_JAVASCRIPT || lang == CBM_LANG_TYPESCRIPT || lang == CBM_LANG_TSX) {
        if (strcmp(pk, "program") == 0) return true;
